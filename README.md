...
natif d'ajouter une FAQ à une page d'archive de catégorie. Saito FAQ comble
...
- **Thème par question (optionnel)** : donner le même thème à plusieurs
  ...
  inutiles.
- **Affichage front** :
  - `[saito_faq]` — FAQ du contexte courant (article/page/produit affiché,
    ou page d'archive d'une taxonomie couverte).
  - `[saito_faq_all]` — toutes les FAQ du site, groupées par titre, pour une
...
- **Schéma `FAQPage` (JSON-LD)** généré côté serveur, uniquement là où le
  ...
  catégorie couverte, seulement si `[saito_faq]` est posé dans le contenu
